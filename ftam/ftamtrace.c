...
int FHookRequest (const int sd, void (*tracing)(int sd, char *event, char *fpdu, PE pe, int rw), struct FTAMindication *fti) {
	SBV	    smask;
	struct ftamblk *fsb;
...
void FTraceHook (int sd, char *event, char *fpdu, PE pe, int rw) {
	if (event)
