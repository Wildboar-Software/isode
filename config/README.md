...
This directory is not a library. It holds the Makefile fragment and C header
that describe the host OS, compiler flags, and install paths. The generation
procedure is `isode-gen(8)` (`doc/isode-gen.8.md`).
...
`CONFIG.make` is what every subdirectory Makefile includes (via the `./make`
wrapper). `h/config.h` is what C sources include.
...
- After changing `CONFIG.make`, run `./make clean` before rebuilding so objects
  compiled under a different fragment are not reused.
- `linux.make` is the fragment that currently produces a full `everything` tree.
  `linux-dev.make` is stricter and does not complete under
