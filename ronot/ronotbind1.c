...
int RoAsynBindRequest(OID context, AEI callingtitle, AEI calledtitle,
  ...
  if (bindargpe != NULLPE) {
    if (encode_RONOT_BindArgumentValue(user_data_p, 1, 0, NULLCP, bindargpe) ==
        NOTOK) {
  ...
  result = AcAsynAssocRequest(context, callingtitle, calledtitle, callingaddr,
                              calledaddr, ctxlist, defctxname, prequirements,
                              srequirements, isn, settings, ref, user_data_p,
                              ndata, qos, acc, aci, async);
  if ((*user_data_p) != NULLPE) {
    pe_free((*user_data_p));
  }
  if (result == NOTOK) {
    LLOG(rosap_log, LLOG_NOTICE,
         ("RO-BIND.REQUEST : RoAsynBindRequest failed"));
