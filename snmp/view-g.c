...
static struct community *get_acent (const unsigned int *ip, const int len, const int isnext);
static int  o_viewTrap (OI oi, struct type_SNMP_VarBind *v, int offset);
static struct trap *get_trent (const unsigned int *ip, const int len, const int isnext);
...
static struct view *get_prent (const unsigned int *ip, const int len, const int isnext);
