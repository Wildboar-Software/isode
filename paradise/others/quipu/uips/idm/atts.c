...
void printDetailsNumbers(int objectType, struct namelist *lp, int *count) {
	Attr_Sequence at;
	AV_Sequence av;
	char * cp;
...
void de_phone_print (PS ps,char *str,int format) {
	if (*str == T61_MARK) {
		if (format != READOUT) {
			ps_print (ps,"{T.61}");
...
static void de_fax_print (PS ps, struct fax *f, int format) {
	int   i;
	struct pair *p;
	PE    pe;
