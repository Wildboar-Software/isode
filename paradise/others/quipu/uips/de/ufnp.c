...
static int countMatches(DNS dns);
static char exact_match (DN dn,char *s);
static char good_match (DN dn,char *s);
static char present (DN d,AttributeType t);
int ufn_search (DN base, char subtree, Filter filt, DNS *res, char *s, DNS (*interact)(DNS, DN, char *), DNS el, int leafFlag);
...
int dnSelect (char *s,DNS *dlist,DNS (*interact)(DNS, DN, char *),DNS el, int leafFlag) {
	DNS exact = NULLDNS;
	DNS good  = NULLDNS;
	DNS bad   = NULLDNS;
