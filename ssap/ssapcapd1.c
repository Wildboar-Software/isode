...
#include <signal.h>
#include "spkt.h"
static int SCapdRequestAux (struct ssapblk *sb, const char *data, const int cc, struct SSAPindication *si);
...
int SCapdRequest (const int sd, const char *data, const int cc, struct SSAPindication *si) {
	SBV	    smask;
	int     result;
	struct ssapblk *sb;
