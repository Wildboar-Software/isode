...
#define PACKBUFFERSIZE 4096
	 ...
	 *
	 */
	static _doXRead16 (Display *dpy, short *data, long size, char *packbuffer) {
		long *lpack,*lp;
	 ...
	 * This routine may have to be reworked if int < long;
	 */
	void _XSend (Display *dpy, char *data, long size) {
		struct iovec iov[3];
...
#define InsertIOV(pointer, length) \
	...
	_SetLastRequestRead(Display *dpy, xGenericReply *rep) {
	 ...
	 * we may encounter.
	 */
	Status _XReply (Display *dpy, xReply *rep, int extra /* number of 32-bit words expected after the reply */, Bool discard /* should I discard data followind "extra" words? */) {
	...
	static _EatData (Display *dpy, unsigned long n) {
	...
	/*ARGSUSED*/
	Bool
	_XUnknownWireEvent(Display *dpy /* pointer to display structure */, XEvent *re /* pointer to where event should be reformatted */, xEvent *event /* wire protocol event */) {
#ifdef notdef
		fprintf(stderr,
	...
	/*ARGSUSED*/
	Status
	_XUnknownNativeEvent(Display *dpy /* pointer to display structure */, XEvent *re /* pointer to where event should be reformatted */, xEvent *event /* wire protocol event */) {
#ifdef notdef
		fprintf(stderr,
	...
	int _XPrintDefaultError (Display *dpy, XErrorEvent *event, FILE *fp) {
		char buffer[BUFSIZ];
		char mesg[BUFSIZ];
		char number[32];
	...
	static void doData16(Display *dpy, short *data, unsigned len, char *packbuffer) {
		long *lp,*lpack;
		long i, nwords,bits;
		long mask16 = 0x000000000000ffff;
	...
	void Data16 (Display *dpy, short *data, unsigned len) {
		char packbuffer[PACKBUFFERSIZE];
		unsigned nwords = (PACKBUFFERSIZE >> 1);	/* bytes to CARD16 */
	...
	static void doData32 (Display *dpy, long *data, unsigned len, char *packbuffer) {
		long *lp,*lpack;
		long i,bits,nwords;
		long mask32 = 0x00000000ffffffff;
	...
	void Data32 (Display *dpy, short *data, unsigned len) {
		char packbuffer[PACKBUFFERSIZE];
		unsigned nwords = (PACKBUFFERSIZE >> 2);	/* bytes to CARD32 */
